    ...
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((item, idx) => {
        ...
        return (
          <Card
            key={idx}
            className="overflow-hidden border border-border shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 ease-out transform animate-in fade-in slide-in-from-bottom-4 duration-500"
...
export function RevenueChart({ data }: RevenueChartProps) {
  return (
    <Card
      className="col-span-4 border border-border shadow-xs transition-all duration-500 ease-out transform animate-in fade-in slide-in-from-bottom-4 duration-500"
...
export function StatusPieChart({ data }: StatusPieChartProps) {
  ...
  return (
    <Card
      className="col-span-4 lg:col-span-3 border border-border shadow-xs transition-all duration-500 ease-out transform animate-in fade-in slide-in-from-bottom-4 duration-500"
            ...
            {activity.recentOrders.length === 0 ? (
              ...
              <div className="divide-y border border-border rounded-xl bg-background overflow-hidden">
                {activity.recentOrders.map((ord, idx) => (
                  <div
                    key={ord._id}
                    className="flex justify-between items-center p-4 hover:bg-slate-50/50 transition-colors duration-200 transform animate-in fade-in slide-in-from-left-4 duration-300"
              ...
              <div className="divide-y border border-border rounded-xl bg-background overflow-hidden">
                {activity.recentReviews.map((rev, idx) => {
                  ...
                  return (
                    <div
                      key={rev._id}
                      className="p-4 hover:bg-slate-50/50 transition-colors duration-200 transform animate-in fade-in slide-in-from-left-4 duration-300"
                      style={{ animationDelay: `${idx * 50}ms` }}
                    >
...
- Run compile check: `pnpm tsc --noEmit`

### Manual Verification Scenarios

1. **Layout Shift (CLS) Testing:** Trigger tab clicks (Orders $\rightarrow$ Users $\rightarrow$ Reviews). Verify that the dashboard height and surrounding elements remain perfectly static (prevented by `min-h-[435px]` heights on tab panels).
2. **Smooth Transitions:** Refresh the page and inspect the visual flow. Cards should slide up (`translate-y-4` to `translate-y-0`) and fade in. Tab switching should feel fluid.
3. **Aggregated Stats Accuracy:** Open `/admin` and cross-verify that the metrics cards display revenue calculations correctly (only paid order values summed).
4. **Sales Chart Responsiveness:** Resize the viewport and verify that the area chart adjusts its scale responsively.
5. **Operational Distribution:** Verify that the status pie chart displays accurate counts matching the active orders in the database.
